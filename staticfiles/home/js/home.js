...
function handleNumberSubmission() {
    ...
    if (numberValue.match('^(\\+98|0)?9\\d{9}$')) {
        fetchCompany(apiKey).then((companyName) => {
            fetchUserIP().then((userIP) => {
                array.push({'userEmail': userIP, 'userNumber': numberValue, 'companyName': companyName});
                connect(userIP, numberValue, companyName);
            }).catch(error => {
...
function connect(email, number, company) {
    fetch('/connect/', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        ...
        body: JSON.stringify({'userEmail': email, 'userNumber': number, 'company': company}),
    })
        .then(res => res.json())
        .then(data => {
            console.log(data);
